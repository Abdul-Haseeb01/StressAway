...
export class ChatbotService {
    ...
    constructor(
        ...
        if (geminiKey) {
            ...
            this.logger.warn('No Gemini API key configured, chatbot will use fallback responses.');
        }
    }
    ...
    private async generateResponse(userId: string, currentMessage: string): Promise<string> {
        ...
        // Fetch user data context (stress scores, etc.)
        const userDataContext = await this.getUserContext(userId);

        // Fetch last 5 messages for conversation flow
        const history = await this.getChatHistory(userId, 6); // 6 to get 3 rounds of convo
        const conversationHistory = history
            .map(m => `${m.is_user_message ? 'User' : 'Assistant'}: ${m.message}`)
            .join('\n');

        const systemPrompt = `You are the StressAway Mental Wellness Assistant. 
You provide empathetic, supportive advice based on the user's current stress data and conversation history.

Current User Data Context (Stress Scores):
${userDataContext}
...
Instructions:
1. Maintain the flow of conversation. If the user is following up on a previous point, acknowledge it.
2. Provide short, bulleted tips for relief if they seem stressed.
3. Be friendly, approachable, and non-judgmental.
4. Suggest platform features (Wellness Activities, Questionnaire, Facial Scan, SOS) ONLY if relevant to the current conversation phase.
5. Be concise and use markdown formatting.`;
